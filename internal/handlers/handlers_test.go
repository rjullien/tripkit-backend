...
func TestTrips_UpdateData(t *testing.T) {
	...
	doReq(r, "PUT", "/api/trips/trip-d", map[string]any{"data": map[string]any{"travelers": []string{"Alice", "Bob"}}})
...
func TestLists_PersonalOwner(t *testing.T) {
	...
	w := doReqAs(r, "PUT", "/api/trips/tp/lists/pl-rene",
		map[string]any{"type": "packing", "title": "Alice's list", "data": map[string]any{}, "owner_user": "rene"},
		"rene")
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d: %s", w.Code, w.Body.String())
	}
...
func TestLists_FilterByOwner(t *testing.T) {
	...
	doReqAs(r, "PUT", "/api/trips/tf/lists/rene-1",
		map[string]any{"type": "packing", "title": "Alice", "data": map[string]any{}, "owner_user": "rene"},
		"rene")

	// Bob's personal list
	doReqAs(r, "PUT", "/api/trips/tf/lists/bob-1",
		map[string]any{"type": "packing", "title": "Bob", "data": map[string]any{}, "owner_user": "bob"},
		"bob")

	// Filter by ?owner=rene: should return shared + rene's list, NOT bob's
...
func TestSync_PersonalListForbidden(t *testing.T) {
	...
	// Bob tries to sync Alice's personal list → 403
	w := doReqAs(r, "PATCH", "/api/trips/tpf/lists/rene-list/sync",
		map[string]any{"deviceId": "bob-phone", "lastSyncAt": 0, "checks": map[string]any{}, "custom": map[string]any{}, "hidden": []any{}},
		"bob")
...
func TestSync_SharedListAnyone(t *testing.T) {
	...
	// Bob can sync the shared list → 200
	w := doReqAs(r, "PATCH", "/api/trips/tsa/lists/shared-list/sync", syncBody, "bob")
	if w.Code != http.StatusOK {
		t.Errorf("expected 200 for bob on shared list, got %d: %s", w.Code, w.Body.String())
	}
...
func TestDeleteList_PersonalForbidden(t *testing.T) {
	...
	doReqAs(r, "PUT", "/api/trips/tdp/lists/rene-del",
		map[string]any{"type": "packing", "title": "Alice del", "data": map[string]any{}, "owner_user": "rene"},
		"rene")

	// Bob tries to delete Alice's list → 403
	w := doReqAs(r, "DELETE", "/api/trips/tdp/lists/rene-del", nil, "bob")
	if w.Code != http.StatusForbidden {
		t.Errorf("expected 403, got %d: %s", w.Code, w.Body.String())
	}
