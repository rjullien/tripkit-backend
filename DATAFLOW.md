...
**Every trip MUST have a `day: 0` entry** representing the day before departure (prep/valises).

- `startDate` = **the first real day of the trip** (Day 1 = travel/arrival)
- `day 0` = veille = computed as `startDate - 1` by the frontend
- The frontend's DayResolver uses: `startDate = Day 1, not Day 0`
- Day 0 is always prep/checklist/valises content

**Date formula in the app:**
```
...
**Example:**
```
startDate = "2026-06-24"   → Day 1 = 24 juin (vol)
                            → Day 0 = 23 juin (veille, valises)
                            → Day 2 = 25 juin
```

**Template for Day 0:**
```javascript
{ day: 0, emoji: "📋", label: "Veille de départ — Checklist & valises",
  timeline: [
    { t: "🧳", d: "Préparer les valises (voir checklist)" },
    { t: "📋", d: "Vérifier documents, adaptateurs, CB" },
    { t: "✈️", d: "Confirmation transport lendemain" },
    { t: "🌤️", d: "Checker météo destination" }
  ]
}
```
